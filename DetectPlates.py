...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True: # show steps #######################################################
        ...
        for i in range(0, len(listOfPossiblePlates)):
            p2fRectPoints = cv.boxPoints(listOfPossiblePlates[i].rrLocationOfPlateInScene)
            ...
            cv.imshow("4b", listOfPossiblePlates[i].imgPlate)
            cv.waitKey(0)
        # end for
...
def findPossibleCharsInScene(imgThresh):
    ...
    contours, npaHierarchy = cv.findContours(imgThreshCopy, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)   # find all contours
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        if Main.showSteps == True: # show steps ###################################################
            cv.drawContours(imgContours, contours, i, Main.SCALAR_WHITE)
        # end if # show steps #####################################################################
